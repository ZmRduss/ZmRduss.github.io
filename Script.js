    ...
    socket.addEventListener('message', (event) => {
      ...
      switch (data[0]) {
        case 5: {
          ...
          if (params.get('private-mode') !== "true") { socket.send(`42[11,"${playerId}","Zlo přichází"]`); }
  ...
  if (socketList) {
    ...
    socketList.forEach(function (socket) {
      if (socket.readyState === WebSocket.OPEN) {
        if (params.get('private-mode') === "true") { socket.send(`42[11,"${socket.playerId}","Zlo přichází"]`); }
        socket.close();
        socket.onerror = null;
        socket.onclose = null;
      }
